...
def evligen(title,start_datetime,end_datetime,location,discription):
    ...
    main_url='https://calendar.google.com/calendar/render?action=TEMPLATE&dates={}%2F{}&details={}&location={}&text={}'
    title=urldecode(title)
    location=urldecode(location)
    discription=urldecode(discription)
    
    start_datetime=jdt.strptime(start_datetime,"%Y/%m/%d-%H:%M").togregorian()
    start_datetime=dt(start_datetime.year,start_datetime.month,start_datetime.day,start_datetime.hour,start_datetime.minute)
    start_datetime=start_datetime.replace(tzinfo=timezone('Iran')).astimezone(tz=timezone('UTC'))
    start_datetime=dt(start_datetime.year,start_datetime.month,start_datetime.day,start_datetime.hour,start_datetime.minute-4)
...
token = '1687593159:AAH_VLwlEPBkA7BxfPmWUzVigyDY_Vr06ec'
...
sample_event_list=sample_event.split('\n')
sample_event_list.remove(sample_event_list[0])
sample_event_link=evligen(sample_event_list[1],sample_event_list[3],sample_event_list[4],sample_event_list[2],'\n'.join(sample_event_list[5:]))
...
def handle_start(message):
    if message.chat.type == 'group':
        bot.reply_to(message,"Welcome to the group, i am your Goolge event generator assistant")
    else:
        bot.reply_to(message, "سلام :) \n من اینجا هستم تا بتونی رویداد هایی رو که میبینی به تقویم گوگلت اضافه کنی.\n برای اینکه بدونی چطوری کار میکنم /help رو وارد کن.")
...
def webhook():
    bot.remove_webhook()
    bot.set_webhook(url='https://event-generato-bot.herokuapp.com/' + token)
    return "!", 200
